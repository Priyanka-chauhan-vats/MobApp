...
<Picker
              ...
              onValueChange={(itemValue, itemPosition) =>
                this.setState({ language: itemValue, choosenIndex: itemPosition })}>
              { officer.map((park, index) => {
                return  <Picker.Item key={index} value={park.Authority_name} label={park.Authority_name} />
...
onValueChange={(itemValue, itemPosition) =>
this.setState({ nodalofficer: itemValue, choosenIndex2: itemPosition })}   >
                    <Picker.Item label="Select" value="Select" />
                                 { officer.map((park, i) => {
if(this.state.choosenIndex ===i){
  return <Picker.Item key={i} value={park.OFFICER_NAME} label={park.OFFICER_NAME} />
}
else{
...
<Block style={{ borderBottomColor: '#356FE0',  padding: 2 }}>
<Text style={styles.text2} >Concerned Nodal Officer Serial Number</Text>
<View style={styles.textInput}>
<Picker
selectedValue={this.state.nodalofficerserialnumber}
onValueChange={(itemValue, itemPosition) =>
this.setState({ nodalofficerserialnumber: itemValue, choosenIndex2: itemPosition })}   >
                    <Picker.Item label="Select" value="Select" />
                                 { officer.map((park, i) => {
if(this.state.choosenIndex ===i){
  return <Picker.Item key={i} value={park.SerialNo} label={park.SerialNo} />
}
else{
  return <Picker.Item label="Select " value="Select" />
}  })}
